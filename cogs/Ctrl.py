...
class Ctrl(commands.Cog):
    ...
    async def cog_check(self, context: commands.Context) -> bool:
        ...
        return await commands.check_any(
            commands.is_owner(), commands.has_role(int(conf["Roles"]["bot ctrl"]))
        ).predicate(context)

    async def cog_handler(
        self, context: commands.Context, pre: Literal["re", "un", ""], *cogs: str
    ) -> None:
        """
        ...
        if pre and (not_loaded := cogs - loaded):
            msg += (
                f"Can't {pre}load cogs that aren't loaded: {', '.join(not_loaded)}.\n"
            )
            cogs -= not_loaded
