...
class Roles(commands.Cog):
    ...
    async def verify(
        self, context: commands.Context, members: commands.Greedy[discord.Member]
    ) -> None:
        """
        Shortcut for giving the verification role.
        """
        for member in members:
            await member.add_roles(self.R_verified, reason=f"Added by {context.author}")
            await member.remove_roles(
                self.R_unverified, reason=f"Removed by {context.author}"
            )
